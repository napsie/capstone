...
<div class="page-shell">
    ...
    <?php if ($pages > 1): $query = ($search !== '' ? '&search=' . rawurlencode($search) : '') . ($idStatus !== 'all' ? '&id_status=' . rawurlencode($idStatus) : ''); ?><nav class="pagination" aria-label="Digital ID pages"><a class="<?= $page <= 1 ? 'disabled' : '' ?>" href="?page=<?= max(1,$page-1) . $query ?>">Previous</a><span>Page <?= $page ?> of <?= $pages ?></span><a class="<?= $page >= $pages ? 'disabled' : '' ?>" href="?page=<?= min($pages,$page+1) . $query ?>">Next</a></nav><?php endif; ?>
</div></main>
