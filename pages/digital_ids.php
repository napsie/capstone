...
<div class="page-shell">
    ...
    ?><nav class="pagination" aria-label="Digital ID pages"><a class="<?= $page <= 1 ? 'disabled' : '' ?>" href="?page=<?= max(1,$page-1) . $query ?>">Previous</a><?php if ($startPage > 1): ?><a href="?page=1<?= $query ?>">1</a><?php if ($startPage > 2): ?><span class="ellipsis">…</span><?php endif; ?><?php endif; ?><?php for ($number = $startPage; $number <= $endPage; $number++): ?><?php if ($number === $page): ?><span class="current" aria-current="page"><?= $number ?></span><?php else: ?><a href="?page=<?= $number . $query ?>"><?= $number ?></a><?php endif; ?><?php endfor; ?><?php if ($endPage < $pages): ?><?php if ($endPage < $pages - 1): ?><span class="ellipsis">…</span><?php endif; ?><a href="?page=<?= $pages . $query ?>"><?= $pages ?></a><?php endif; ?><a class="<?= $page >= $pages ? 'disabled' : '' ?>" href="?page=<?= min($pages,$page+1) . $query ?>">Next</a></nav><?php endif; ?>
</div></main>
