        ...
        elseif ($file['size'] > 5 * 1024 * 1024) $errors[] = 'The file must not exceed 5 MB.';
            ...
            try {
                ...
                foreach ($records as $record) {
                    ...
                    $preview[] = ['row'=>(int)$record['_row'],'application_type'=>$type,'senior_id_no'=>normalizeSeniorId($seniorId),'full_name'=>normalizePersonName($record['full_name'] ?? ''),'birth_date'=>$birth ?: '','contact_number'=>normalizePhoneNumber($record['contact_number'] ?? ''),'complete_address'=>normalizeWhitespace($record['complete_address'] ?? ''),'barangay'=>normalizeBarangayName($record['barangay'] ?? '', $barangays_list),'email_address'=>strtolower(trim($record['email_address'] ?? '')),'date_submitted'=>$submitted,'errors'=>$rowErrors];
                }
